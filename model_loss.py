...
#         # targets 转换成 one-hot 编码
#         targets = torch.eye(self.num_classes).cuda()[targets]
#         bce_loss = F.cross_entropy(inputs, targets, reduction=self.reduction)
#         modulating_factor = (1 - probs).pow(self.gamma)
#         alpha_weight = self.alpha * targets + (1 - self.alpha) * (1 - targets)
#         focal_loss = modulating_factor * alpha_weight * bce_loss
...
#             1 - targets_one_hot
#         )
#         focal_loss = modulating_factor * alpha_weight * ce_loss
#         if self.reduction == "mean":
#             return torch.mean(focal_loss)
#         elif self.reduction == "sum":
#             return torch.sum(focal_loss)
